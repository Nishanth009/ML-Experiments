...
def predict(x,W):
    ...
    for i in range(samples):
        score = np.dot(x[i],W)
        pred = float(1)/float(1+math.exp(-(score)))
...
def calc_error(x,y,w1,w2,w0):
    error = 0
    thr = 0.5
    pred = float(1)/float(1+math.exp(-(w0+x[0]*w1+x[1]*w2)))
    if round(pred,3)>=round(thr,3):
        out = 1
    else:
        out = 0
...
def computeWeights(X,Y):
    ...
    for i in range(iterations):
        for xval,yval in zip(X,Y):
            pred,error = calc_error(xval,yval,w1,w2,w0)
            dw0 = error*pred*(1-pred)
            dw1 = (xval[0]*error*pred*(1-pred))
            dw2 = (xval[1]*error*pred*(1-pred))
            w0+=alpha*dw0
            w1+=alpha*dw1
            w2+=alpha*dw2
    return w0,w1,w2
...
if __name__=='__main__':
    ...
    ##MODEL & PREDICT (logistic classifier - binary classification problem)
